                  ...
                  <div class="row">
                    <div class="col-md-12 col-lg-3 col-xl-3 mb-4 mb-lg-0">
                      <div class="bg-image hover-zoom ripple rounded ripple-surface">
                        <img :src="item.service[0].image" class="w-100" />
                        <a href="#!">
                    ...
                    <div class="col-md-6 col-lg-3 col-xl-3 border-sm-start-none border-start">
                      ...
                      <em class="text-secondary">( &#8377;{{ item.service[0].price }} + &#8377;{{
                        item.service[0].price * .08 }} gst )</em>
                      <div>
                        <div v-if="item.serviceStatus == 'Requested'" class="d-flex flex-column mt-4">
                          <button class="btn btn-outline-success btn-sm mt-2" type="button"
                            v-on:click="acceptService(item._id)" :disabled="item.serviceStatus == 'Cancelled' ? true : false">
                            Accept Request
                          </button>
                          <button class="btn btn-outline-danger btn-sm mt-2" type="button"
                            v-on:click="cancelService(item._id)" :disabled="item.serviceStatus == 'Cancelled' ? true : false">
                            Cancel Request
                          </button>
                        </div>
                        <div v-else-if="item.serviceStatus == 'Accepted'" class="d-flex flex-column mt-4">
                          <button class="btn btn-outline-success btn-sm mt-2" type="button"
                            v-on:click="completed(item._id)" :disabled="item.serviceStatus == 'Success' ? true : false">
                            Service Served
                          </button>
                        </div>
                        <div v-else-if="item.serviceStatus == 'Cancelled'" class="bg-image ripple rounded ripple-surface">
                          <img src="@/image/cancle.png" class="w-50" />
                          <a href="#!">
                            <div class="mask" style="background-color: rgba(253, 253, 253, 0.15);"></div>
                          </a>
                        </div>
                        ...
                        <div v-else class="bg-image ripple rounded ripple-surface">
                          <img src="@/image/success.png" class="w-50" />
                          <a href="#!">
                            <div class="mask" style="background-color: rgba(253, 253, 253, 0.15);"></div>
                          </a>
                        </div>
